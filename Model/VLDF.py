...
class VLDF_coef(object):
    def __init__(self, alpha = 100, beta = 100, chi = 1, gamma = 100, D = 100, L = 1, optimize = 'Adam_update'):
...
class VLDF_params(object):
    ...
    def update_value(self, params_name, params_value, struct):
        ...
        params_num = [len(struct.encoder3.share.activation), len(struct.encoder3.mu.activation), len(struct.encoder3.sigma.activation)]
        for k in range(len(params_num)):
            tmp=[]
            for j in range(params_num[k]):
                tmp.append(params[i])
                ...
                tmp.append(params[i])
                i = i+1
            self.EC3_params.append(tmp)          

        params_num = [len(struct.encoder4.share.activation), len(struct.encoder4.mu.activation), len(struct.encoder4.sigma.activation)]
        for k in range(len(params_num)):
            tmp=[]
            for j in range(params_num[k]):
                tmp.append(params[i])
                ...
                tmp.append(params[i])
                i = i+1
            self.EC4_params.append(tmp)              
            
        for j in range(len(struct.encoder5.activation)):
            self.EC5_params.append(params[i])
            i = i+1        
            self.EC5_params.append(params[i])
            i = i+1        
        
        for j in range(len(struct.encoderX.activation)):
            self.ECX_params.append(params[i])
            i = i+1        
            self.ECX_params.append(params[i])
            i = i+1        
        ...
        params_num = [len(struct.decoder3.share.activation), len(struct.decoder3.mu.activation), len(struct.decoder3.sigma.activation)]
        for k in range(len(params_num)):
            tmp=[]
            for j in range(params_num[k]):
                tmp.append(params[i])
...
#------------------------------------------------------------------------
        ...
        # KL(q(zd)||p(zd)) -----------
        self.KL_zd_source = er.KLGaussianGaussian(self.EC_zd_S_mu, self.EC_zd_S_log_sigma, self.DC_zd_S_mu, self.DC_zd_S_log_sigma).sum()
        self.KL_zd_target = er.KLGaussianGaussian(self.EC_zd_T_mu, self.EC_zd_T_log_sigma, self.DC_zd_T_mu, self.DC_zd_T_log_sigma).sum()
        ...
        # KL(q(ad)||p(ad)) -----------     
        self.KL_ad_source = er.KLGaussianStdGaussian(self.EC_ad_S_mu, self.EC_ad_S_log_sigma).sum()
        self.KL_ad_target = er.KLGaussianStdGaussian(self.EC_ad_T_mu, self.EC_ad_T_log_sigma).sum()
        ...
        self.LH_d_source = - T.sum(- d_source * T.log( T.maximum(self.EC_d_S_pi, threshold)), axis=1).sum()        
        self.LH_d_target = - T.sum(- d_target * T.log( T.maximum(self.EC_d_T_pi, threshold)), axis=1).sum()
        
        
        # MMD betwween s, x using gaussian kernel-----------
        #self.MMD = MMD(self.zy_S, self.zy_T, batch_size)
        self.MMD = er.MMDEstimator(rng, self.zy_S, self.zy_T, zy_dim, batch_size, D).sum()
    ...
    def domain_feature_outputs(self):
        return [self.EC_zd_S_mu, self.EC_zd_T_mu]
...
def VLDF_training(source_data, target_data, n_train_batches, n_epochs, struct, coef, description, process_display=True):
    ...
    features_model = theano.function(
        ...
        [feature_classifier.source_reconstruct(), feature_classifier.target_reconstruct()] + feature_classifier.domain_feature_outputs(),
        givens={
